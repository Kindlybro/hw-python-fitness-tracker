class InfoMessage:
    ...
    def get_message(self) -> str:
        return (f'Тип тренировки: {self.training_type}; '
                f'Длительность: {round(self.duration, 3):.{3}f} ч.; '
                f'Дистанция: {round(self.distance, 3):.{3}f} км; '
                f'Ср. скорость: {round(self.speed, 3):.{3}f} км/ч; '
                f'Потрачено ккал: {round(self.calories, 3):.{3}f}.')
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        return ((coeff_calorie_1 * self.weight
                 + (self.get_mean_speed() ** coeff_calorie_2 // self.weight)
